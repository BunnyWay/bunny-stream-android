 ...
 *
 * Every failed management call collapses into one of five cases. The taxonomy is deliberately
 * small: callers almost always branch on two questions only — "was it the network or the server"
 * and "is it worth retrying" — and both answers are derivable from every variant:
 *
 *  * [httpStatus] carries the numeric HTTP status of the failed call, or `0` when no usable HTTP
 *    response existed at all ([Network], [Decode]).
 *  * [isTerminal] tells whether retrying can ever succeed. It is `true` exactly for
 *    `401`, `403`, `404` and `410` — the same set the live-stream polling loop has always used
 *    (`410 Gone` is what Bunny returns for a deleted library; once seen, it never recovers).
...
public sealed class BunnyError {
     ...
     * Transient failures — `5xx`, [Network], [Decode] — return `false`.
     */
    ...
    public companion object {
        /** [httpStatus] value meaning "no usable HTTP response existed" ([Network], [Decode]). */
